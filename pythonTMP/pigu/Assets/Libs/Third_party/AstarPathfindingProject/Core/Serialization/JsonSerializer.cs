...
namespace Pathfinding.Serialization {
	...
	public class AstarSerializer {
        ...
        static byte[] SerializeServerGraphData(NavGraph graph)
        {
            var stream = new MemoryStream();
            var writer = new BinaryWriter(stream);
            var ctx = new GraphSerializationContext(writer);

            graph.SerializeServerInfo(ctx);
		...
		public void SerializeExtraInfo () {
            ...
            for (int i = 0; i < graphs.Length; i++) {
                ...
                bytes = SerializeServerGraphData(graphs[i]);
                AddChecksum(bytes);
                AddEntry("graph" + i + "_server" + binaryExt, bytes);
