...
function ZEntryModuleData.send_CS_Login(token)
	local CS_Login = {}
	CS_Login.token = token
	GameState.tcpClinet.sendmsg(Cmd.CS_Login,CS_Login)
end
function ZEntryModuleData.send_CS_LoginByAccount(account,password)
...
function ZEntryModuleData.send_CS_RegistByPhone(phone,code,password,uid)
